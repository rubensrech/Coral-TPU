...
log = logging.getLogger("Conv2dModelCreator")
...
def create_conv2d_tflite_model(kernel: tf.Tensor, input_shape: Tuple[int]):
    shape_str = "_".join(map(str, input_shape))
    model_name = "conv2d_%s" % (shape_str)
...
def create_edgetpu_model(input_shape: Tuple[int], model_name: str, model_func, keep_only_op_codes=["CONV_2D"]):
...
def create_conv2d_edgetpu_model(kernel: tf.Tensor, input_shape: Tuple[int]):
    shape_str = "_".join(map(str, input_shape))
    model_name = "conv2d_%s_quant" % (shape_str)
...
def create_conv2d_model(input_size: Tuple[int], kernel_size: Tuple[int], kernel_type: Kernel, plataform: Plataform):
    ...
    assert kernel is not None, f"Unknown kernel `{kernel_name}`"
    ...
    if plataform == Plataform.TensorFlowLite:
        return create_conv2d_tflite_model(kernel, input_shape)
    elif plataform == Plataform.EdgeTPU:
        return create_conv2d_edgetpu_model(kernel, input_shape)

def main():
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('-I', '--input-size', default="300,300",
    ...
    def create_model_for_plataform(plataform: Plataform):
        model_file = create_conv2d_model(input_size, kernel_size, kernel_type, plataform)
